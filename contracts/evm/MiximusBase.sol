...
/**
 * @title MiximusBase
 * @notice Abstract base contract for the Miximus multi-chain mixer.
 *         Contains all zkSNARK verification logic, Merkle tree management,
 *         and nullifier tracking. Payment handling is delegated to child contracts.
 *
 * Ported from the original ethsnarks-miximus by HarryR, upgraded to Solidity 0.8+
 * and made chain-agnostic for deployment on any EVM-compatible blockchain.
 */
abstract contract MiximusBase {
    // =========================================================================
    //                              CONSTANTS
    // =========================================================================
    ...
    /// @notice The BN254 scalar field modulus used by the zkSNARK circuit
    ...
    /// @notice The fixed denomination for this mixer pool (in wei / smallest unit)
    uint256 public immutable denomination;

    /// @notice Human-readable symbol for the asset being mixed (e.g. "ETH", "USDT")
    string public assetSymbol;

    /// @notice Nullifier tracking — prevents double-spend
    mapping(uint256 => bool) public nullifiers;

    /// @notice Valid Merkle roots — each root after a deposit is recorded
    mapping(uint256 => bool) public roots;

    /// @notice Current number of leaves in the Merkle tree
    uint256 public nextLeafIndex;

    // Merkle tree storage: treeNodes[level][index] = hash
    ...
    constructor(
        ...
        // Initialize the Merkle tree with zero leaves
        _initMerkleTree();
    }
    ...
    /// @notice Retrieve the Merkle authentication path for a given leaf index
    ...
    // =========================================================================
    //                         DEPOSIT / WITHDRAW (core logic)
    // =========================================================================

    /// @dev Internal deposit logic — inserts leaf into Merkle tree
    function _processDeposit(uint256 _leaf) internal returns (uint256 newRoot, uint256 leafIndex) {
        require(nextLeafIndex < MAX_LEAVES, "Merkle tree is full");

        leafIndex = nextLeafIndex;
        nextLeafIndex++;

        // Insert leaf into tree and compute new root
        newRoot = _insertLeaf(_leaf, leafIndex);
        currentRoot = newRoot;
        roots[newRoot] = true;
    ...
    /// @dev Internal batch deposit — inserts multiple leaves in a single transaction
    function _processBatchDeposit(uint256[] calldata _leaves)
        internal returns (uint256 startIndex)
    {
        ...
        for (uint256 i = 0; i < _leaves.length; i++) {
            _processDeposit(_leaves[i]);
        }
    }

    /// @dev Internal batch withdrawal — processes multiple withdrawals in one transaction
    function _processBatchWithdraw(
        uint256[] calldata _roots,
        uint256[] calldata _nullifiers,
        uint256[8][] calldata _proofs,
        address payable _recipient
    ) internal {
        require(_roots.length == _nullifiers.length && _roots.length == _proofs.length, "Array length mismatch");
        require(_roots.length > 0 && _roots.length <= 5, "Batch size must be 1-5");
        for (uint256 i = 0; i < _roots.length; i++) {
            _processWithdraw(_roots[i], _nullifiers[i], _proofs[i], _recipient);
        }
    }

    /// @dev Internal withdrawal logic — verifies proof and marks nullifier
    function _processWithdraw(
        ...
        require(
            _verifyProof(_root, _nullifier, exthash, _proof),
    ...
    /// @notice Groth16 verification on BN254 using EVM precompiles (EIP-196/197)
    function _groth16Verify(
        ...
        // Compute the linear combination vk_x = gammaABC[0] + sum(input[i] * gammaABC[i+1])
        uint256[2] memory vkX;
        vkX[0] = _vkGammaABC[0];
        vkX[1] = _vkGammaABC[1];

        for (uint256 i = 0; i < _input.length; i++) {
            // EC scalar mul: input[i] * gammaABC[i+1]
        ...
        // Pairing check: e(A, B) * e(-vkX, gamma) * e(-C, delta) * e(-alpha, beta) == 1
        // Rearranged: e(A, B) == e(alpha, beta) * e(vkX, gamma) * e(C, delta)
        return _pairingCheck(
            _proof[0], _proof[1],                // A
            _proof[2], _proof[3], _proof[4], _proof[5], // B
            _proof[6], _proof[7],                // C
            _vk,                                  // alpha, beta, gamma, delta
            vkX[0], vkX[1]                       // vk_x
        );
    }
    ...
    function _pairingCheck(
        ...
        // Pair 2: (-alpha, beta) — negate alpha
        input[6] = _vk[0];
        input[7] = P - _vk[1]; // negate
        input[8] = _vk[2];
        input[9] = _vk[3];
        input[10] = _vk[4];
        input[11] = _vk[5];

        // Pair 3: (-vkX, gamma) — negate vkX
        input[12] = vkxX;
        input[13] = P - vkxY; // negate
        input[14] = _vk[6];
        input[15] = _vk[7];
        input[16] = _vk[8];
        input[17] = _vk[9];

        // Pair 4: (-C, delta) — negate C
        input[18] = cX;
        input[19] = P - cY; // negate
        input[20] = _vk[10];
        input[21] = _vk[11];
        input[22] = _vk[12];
        input[23] = _vk[13];
    ...
    /// @notice MiMC-p/p cipher with exponent 7, matching the ethsnarks C++ circuit exactly.
    ///         Uses keccak256 hash chain from seed keccak256("mimc") for round constants.
    function _mimcCipher(uint256 in_x, uint256 in_k) internal pure returns (uint256 out_x) {
        ...
        assembly {
            ...
            for { let i := 91 } gt(i, 0) { i := sub(i, 1) } {
                // c = H(c) -- next round constant in the hash chain
    ...
    /// @notice MiMC hash using Miyaguchi-Preneel compression, matching ethsnarks MiMC.Hash()
    function mimcHash(uint256[] memory _data) public pure returns (uint256) {
        return _mimcHashWithIV(_data, 0);
    }

    /// @notice MiMC hash with a custom IV, used for level-specific Merkle tree hashing
    ...
    /// @notice Returns the level-specific IV for Merkle tree hashing, matching ethsnarks C++ circuit
    ...
    function _initMerkleTree() internal {
        // Compute zero hashes for each level, using level-specific IVs
        uint256 zero = 0;
        for (uint256 i = 0; i < TREE_DEPTH; i++) {
            zeroHashes[i] = zero;
    ...
    /// @notice Returns the node hash at a given level and index, or the zero hash if uninitialized
