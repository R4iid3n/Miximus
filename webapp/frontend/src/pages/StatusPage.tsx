...
export default function StatusPage() {
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: 24 }}>
      <h1 style={{ color: '#fff', fontSize: 24, marginBottom: 8 }}>Track Order</h1>
      <p style={{ color: '#888', fontSize: 14, marginBottom: 24 }}>Enter your order ID to check the mixing status.</p>

      <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        <input value={orderId} onChange={(e) => setOrderId(e.target.value)}
          onKeyDown={(e) => e.key === 'Enter' && handleLookup()}
          placeholder="Enter order ID..." style={{
      ...
      {order && (
        <div style={{ background: '#1a1a2e', borderRadius: 12, padding: 20, border: '1px solid #2a2a3e' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
            <div>
              <div style={{ color: '#888', fontSize: 12 }}>Asset</div>
